...
export function ModifyShift() {
  ...
  const deactivateShift = (shiftID) => {
    fetch("api/v1/shift/deactivate-shift/" + shiftID, {
      method: "PATCH",
      headers: {
  ...
  const modifyShiftPutRequest = () => {
    fetch("api/v1/shift/modify-shift/" + params.id, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  useEffect(() => {
    if (
      currentShift.startIntEnum &&
      currentShift.endIntEnum &&
      currentShift.name
    ) {
      ...
      <Container>
        ...
        <Grid item sm={2} marginTop={1}>
          <Stack direction="row" spacing={2}>
            ...
            <Button
              id="delete-button-edit-shift"
              variant="contained"
              onClick={() => deactivateShift(currentShift.id)}
            >
              Ištrinti
            </Button>
            <Link to="/shifts">
              <Button id="back-button-edit-shift" variant="contained">
                Grįžti
              </Button>
            </Link>
          </Stack>
